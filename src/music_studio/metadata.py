...
def preview(value):
    ...
    return dict(song=song, compiled_lyrics=compiled, compiler_version=VERSION,
...
def generation_input(song):
    ...
    song = result['song']
    return dict(title=song['title'], style=song['music_settings']['advanced_prompt'],
...
def validate_snapshot(data):
    ...
    if (data.get('compiled_lyrics') != data.get('lyrics') or
            ...
            song['title'] != data.get('title') or song['music_settings']['advanced_prompt'] != data.get('style')):
